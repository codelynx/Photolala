...
import WebKit
	...
	struct HelpWebView: NSViewRepresentable {
		...
		private func loadHTMLFile(in webView: WKWebView) {
			guard let helpURL = Bundle.main.url(forResource: htmlFileName, withExtension: "html", subdirectory: "Help")
			else {
				print("Could not find help file: \(self.htmlFileName).html")
				return
			}
